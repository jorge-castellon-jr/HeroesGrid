...
export const customRangers = sqliteTable('custom_rangers', {
  id: text('id').primaryKey(),
  userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
...
export const customRangerLikes = sqliteTable('custom_ranger_likes', {
  id: text('id').primaryKey(),
  userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  customRangerId: text('custom_ranger_id').notNull().references(() => customRangers.id, { onDelete: 'cascade' }),
  createdAt: integer('created_at', { mode: 'timestamp' }).$defaultFn(() => new Date()),
}, (table) => ({
  userRangerIdx: index('likes_user_ranger_idx').on(table.userId, table.customRangerId),
}));
